...
if (length(list.files(pattern = "birdTraits_.*\\.csv$")) != 0) {
  ...
  birds_biomes <- read.csv("./trait_datasets/birdTraits/birds_intersection_biomes_2025-07-25.csv") %>% 
  ...
  avonet <- read_excel("./trait_datasets/birdTraits/externalData/AVONET Supplementary dataset 1.xlsx", 
  ...
  santini2023 <- read_csv("./trait_datasets/birdTraits/externalData/geb13688-sup-0003-appendixs2.csv") %>% 
    # select important columns 
  ...
  jetz2008 <- read_excel("./trait_datasets/birdTraits/externalData/jetz2008_avianClutch.xlsx") %>% 
  ...
  Amniote2015 <- read_csv("./trait_datasets/birdTraits/externalData/Amniote_Database_Aug_2015.csv") %>%
  ...
  bird_et_al_2020 <- read_excel("./trait_datasets/birdTraits/externalData/cobi13486-sup-0002-tables2.xlsx") %>% 
    # select important columns 
  ...
  combined_birdTraits <- birds_biomes %>% 
    ...
    distinct() %>% 
    drop_na(c(Clutch, adult_body_mass_g, Predicted_Density_n_km2,
              Q75, Adult_survival_M, Age_at_first_reproduction_M,
              Maximum_longevity_M ))

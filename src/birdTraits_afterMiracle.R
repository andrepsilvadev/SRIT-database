...
birdTraits_2025_03_03 <- read_csv(here("externalData", "birdTraits_2025-03-03.csv"))

birdTraits_IUCN <- birdTraits_2025_03_03 %>% 
  dplyr::select(sci_name, Order, Family, # taxonomic info
                Clutch_MIN, Clutch_MEAN, Clutch_MAX, `Broods per year`, # clutch info
                `Age of first breeding`, `Life span`, # Ages
...
santini2023 <- read_csv(here("externalData","geb13688-sup-0003-appendixs2.csv")) %>% 
...
jetz2008 <- read_excel(here("externalData", "jetz2008_avianClutch.xlsx")) %>% 
...
Amniote2015 <- read_csv(here("externalData","Amniote_Database_Aug_2015.csv")) %>%
...
bird_et_al_2020 <- read_excel("externalData/cobi13486-sup-0002-tables2.xlsx") %>% 
...
birdTraits <- birdTraits_IUCN %>% 
  left_join(santini2023, by = c("sci_name" = "Species")) %>% 
  left_join(jetz2008, by = c("sci_name" = "sci_name")) %>% 
  left_join(bird_et_al_2020, by = c("sci_name" = "Species")) %>% 
  left_join(Amniote2015, by = c("sci_name" = "sci_name")) %>% 
  #left_join(age_longevity_birds, by = c("sci_name" = "Species")) %>% 
  ...
  dplyr::select(# taxonomic info
    ...
    # clutch sizes and nº clutches per year
    #Clutch_MIN, Clutch_MEAN, Clutch_MAX, 
    Clutch, 
    #litter_or_clutch_size_n, `Broods per year`, litters_or_clutches_per_y,
    
    # age of first breeding (days)
    #`Age of first breeding`, female_maturity_d, age_sexual_maturity_days,
    
    # max age
    #`Life span`, maximum_longevity_y, longevity_y, `Lifespan (wild, undetermined) - undetermined - average - years`,
...
taxonomy_df <- read_csv(here("birds_taxonomy.csv"))
...
birdTraits_df <- birdTraits %>% 
  ...
  tidyr::drop_na(!c(sci_name, order, family, Genus, Species)) %>% 
...
write.csv(birdTraits_df, "C:/Users/maria/OneDrive - Universidade de Lisboa/ANDRE/NatPoKe/traits/birdTraits_afterMiracle20250505.csv", row.names = FALSE)
...
write.csv(GBIF_bird_sps[, c("species", "decimalLatitude", "decimalLongitude", "year")],
          "C:/Users/maria/OneDrive - Universidade de Lisboa/ANDRE/NatPoKe/traits/GBIF_birds_30+occurrences.csv",
          row.names = FALSE)
gc()
head(GBIF_bird_sps)
